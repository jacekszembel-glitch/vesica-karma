...
import type { Yoga } from "@/lib/astro/yogas";

/**
 * Dymek z listą jog aktywnych w danej mahadaszy — ten sam pomysł, co panel
 * kontekstowy w „Jogi i szczęście" pod kosmogramem (najedź na planetę,
 * zobacz jej jogi), tylko po najechaniu na okres na osi czasu.
 */
export default function DymekOkresu({ birth, faza, jogi }: { birth: Date; faza: FazaZycia; jogi: Yoga[] }) {
  ...
  return (
    <div style={{
      ...
      {jogi.length === 0 ? (
        <p className="muted" style={{ margin: 0, fontSize: "0.76rem" }}>Żadna wykryta joga nie działa w tym okresie.</p>
      ) : (
        <div style={{ display: "grid", gap: 5 }}>
          {jogi.map((j) => (
            <p key={j.id} style={{ margin: 0 }}>
              <strong>{nazwaZPara(j)}</strong>
              <span className="muted" style={{ display: "block", fontSize: "0.74rem" }}>{j.znaczenie}</span>
              <span style={{ display: "block", fontSize: "0.72rem", color: "var(--sand)", marginTop: 2 }}>
                U Ciebie: {j.uzasadnienie}
              </span>
            </p>
          ))}
        </div>
